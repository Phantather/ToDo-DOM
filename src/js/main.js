        ...
        list.innerHTML += `
                <li class="todo__item">
                    <div>
                        ...
                        <span style="color: ${item.isImportant ? 'darkorange' : ''}; text-decoration: ${item.isDone ? 'line-through' : ''} " class="todo__item-title">${item.title}</span>
                    </div>
...
form.addEventListener('submit', (e) => { // при отправке формы вызываем функцию
    ...
    todoList = [...todoList,
        {
            id: todoList.length ? todoList[todoList.length - 1].id + 1 : 1, //устанавливаем значениекоторая будет больше на 1 чем предыдущий
            title: e.target[0].value, //устанавливаем значение - из данныхкоторый получили при отправке формы
            isDone: false, isImportant: false,
            time: getTime.toString().substr(16, 5)
        }]; // переопределяемсписокдел - гдеговорилвозьмивсечтоутебябылоиовыйобъект
    e.target[0].value = ''; // обнуляем значениеполеввода
    list.innerHTML = '';
    showList()
});
...
important.addEventListener('click', () => {todoStatus.textContent = 'Important';list.innerHTML = '';
showList('important')
});
